...
def save_images_data(dirpath="./flair_235",filename="images_data.h5"):
...
save_images_data(dirpath="./seg_235",filename="seg_data_235.h5")

"""load data and check element values of seg image"""

data_path = "./seg_data_235.h5"
...
def save_tumor_proprotion_data(dirpath="./seg_235",filename="tumor_proportion.h5"):
  """
...
tumor_prop_path = "./tumor_proportion.h5"
h5f = h5py.File(tumor_prop_path,'r')
tumor_prop = h5f['tp'][:]
tumor_prop.shape     # (235,155)

# check the number of tumor slices of the first patient
print(len(np.where(tumor_prop[0]>0)[0]))

def extract_tumor_slices_data(tumor_prop, threshold=0, dirpath="./flair_235"):
  # get all the filepaths
  file_paths = []
  for dirp, sf, files in os.walk(dirpath):
    for file in sorted(files):
